...
function sitemap_entry(string $path, ?string $lastmod=null): array {
  return ['loc'=>absolute_url($path), 'lastmod'=>$lastmod];
}

function sitemap_render_urlset(array $urls, array $extNS=[]): string {
  $ns = ['xmlns'=>'http://www.sitemaps.org/schemas/sitemap/0.9'];
  foreach ($extNS as $k=>$v) $ns['xmlns:'.$k]=$v;
  $attrs=[]; foreach($ns as $k=>$v) $attrs[]=$k.'="'.$v.'"';
  $out=['<?xml version="1.0" encoding="UTF-8"?>','<urlset '.implode(' ',$attrs).'>'];
  foreach($urls as $u){
    $out[]='  <url>';
...
function sitemap_render_videos(array $items): string {
  $ns=['xmlns'=>'http://www.sitemaps.org/schemas/sitemap/0.9','xmlns:video'=>'http://www.google.com/schemas/sitemap-video/1.1'];
  $attrs=[]; foreach($ns as $k=>$v) $attrs[]=$k.'="'.$v.'"';
  $out=['<?xml version="1.0" encoding="UTF-8"?>','<urlset '.implode(' ',$attrs).'>'];
  foreach($items as $it){
    $out[]='  <url>';
...
function sitemap_render_news(array $newsItems, string $publicationName, string $publicationLanguage): string {
  ...
  foreach($newsItems as $n){
    ...
    $out[]='    <news:news>';
    $out[]='      <news:publication><news:name>'.xml($publicationName).'</news:name><news:language>'.xml($publicationLanguage).'</news:language></news:publication>';
...
function sitemap_render_index(array $sitemaps): string {
  ...
  foreach($sitemaps as $loc){ $out[]='  <sitemap><loc>'.xml($loc).'</loc></sitemap>'; }
  $out[]='</sitemapindex>';
  return implode("\n",$out);
}
